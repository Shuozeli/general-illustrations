...
### `tech-diagram` -> style `technical-minimal`
...
### `code-review` -> style `review-minimal`
...
### `article-sketch` -> style `editorial-sketch`
...
### `product-explainer` -> style `product-explainer`
...
### `whiteboard-story` -> style `whiteboard-comic`
...
### `doodle-light` -> style `simple-doodle`
...
### `sticker-layers` -> style `sticker-layer`
...
### `clay-metaphor` -> style `soft-clay-cartoon`
...
### `software-news` -> style `cartoon-explainer`
...
### `consumer-coupon` -> style `coupon-worker-comic`
...
### `finance-news` -> style `yellow-worker-comic`
...
### `infra-meme` -> style `mambo-meme-comic`
...
### `story-infra` -> style `story-infra-kafka-mambo-technical`

```text
Generate one 16:9 horizontal flat 2D story-infrastructure comic frame. White or very light background, thick black outlines, original abstract system-component characters (brokers, partitions, replicas, consumers, logs) acting out one clear mechanism beat in a readable storyboard. Big empty space, main action 50-70%, restrained red/yellow/blue accents; the Kafka/queue mechanism (offset, consumer group, retry, idempotency, ISR) must stay legible. No existing meme/IP/emoji, no danmaku, no text paragraphs. No text, no words, no logos, no watermark. Scene: {scene}
```

### `honglou-keyframe` -> style `honglou-retro-flat-anime`
...
### `honglou-painterly` -> style `honglou-period-drama-painterly`

```text
Generate one 16:9 full-bleed painterly period-drama keyframe, no border, no margins, TV-drama still feel. Original mature Qing-era-inspired character portraits with realistic adult proportions and serious expressions, soft painterly shading, muted classical palette, period mansion / ancestral hall / study / courtyard settings reduced to readable shapes. Do not copy any real actor, existing drama still, or IP character. Not chibi, not flat cartoon, not 3D/Pixar, no modern clothing or devices. No in-image title, subtitle, text, logo or watermark. Scene: {scene}
```
