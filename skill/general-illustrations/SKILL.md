...
所有图片相关 Recipe 都应该优先写在本仓库的 specs/general-illustrations.json 中，再渲染到 skill/general-illustrations。不要在视频项目、文章项目或临时 run 目录里长期维护独立图片 Recipe；那些地方最多保留项目级引用、样例或产物记录。
...
- `references/recipes.md`：配方目录：类别 → 风格 + 构图 + 模板 + Gemini/ChatGPT 提示词。
- `references/providers/gemini.md`：Gemini provider 集：每个配方忠于风格的 Gemini 提示词 + CDP 使用契约。
- `references/providers/chatgpt.md`：ChatGPT provider 集：每个配方的 ChatGPT 提示词（override 或 Gemini 回退）+ CDP 契约。
