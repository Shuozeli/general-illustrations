...
### story-infra (`story-infra`)

Tags: video / tech / kafka / infrastructure / series

适合：Kafka / 消息队列 / 分布式系统中文短视频系列：事故开场、消息流、分区、副本、offset、consumer group、重试、幂等、ISR 机制。

- 风格 style：`story-infra-kafka-mambo-technical`
- 默认构图 composition：`action-loop-scenes`
- 提示词模板 template：`story-infra-kafka-video-frame`
...
### honglou-painterly (`honglou-painterly`)

Tags: video / literary / honglou / period-drama / painterly

适合：红楼梦文学视频需要更接近真人演员转绘、电视剧剧照感、成熟人物肖像和严肃家族叙事时使用。

- 风格 style：`honglou-period-drama-painterly`
- 默认构图 composition：`state-scenes`
- 提示词模板 template：`honglou-period-drama-video-frame`
